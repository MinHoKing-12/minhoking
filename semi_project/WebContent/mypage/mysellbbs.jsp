				...
				<div class="row gtr-200">
						...
						<div id="sidebar">
							...
							<section>
								<ul class="links">
									...
									<li class="active"><a href="mypagecontrol?command=myfreebbs">> 나의 게시글</a></li>
					...
					<div class="col-8  col-12-narrower imp-narrower">
						<div id="content">
							<div>
								<ul class="tabs">
									<li id="myfreebbs">자유게시판</li>
									<li id="mysellbbs" class="current">거래게시판</li>
								</ul>
							</div>
							
							<div>
								<%
								List<sell_Dto> list = (List<sell_Dto>)request.getAttribute("mysellList");
								%>
								<table border="1" class="type01">
								<col width="70"><col width="80"><col width="500"><col width="80"><col width="150"><col width="200"><col width="70"><col width="70">
								<tr class="tb_head">
								   <th>번호</th>
								   <th>분류</th>
								   <th>제목</th>
								...
								if(list == null || list.size() == 0){  //글이 하나도 없을때
								   %>
								   <tr>
								      <td colspan="8" align="center">작성된 글이 없습니다</td>
								   </tr>
								   <%
								}else{
								   for(int i = 0; i < list.size(); i++){
									   sell_Dto bbs = list.get(i); 
									   
								   %>
								   
								   <tr>
								      <th><%=i + 1 %></th>
								      <td align="center"><%=bbs.getCategory() %></td>
								       <td class="seqclick" >        
								         <a href="sellcontrol?command=detail&seq=<%=bbs.getSeq() %>"><%=bbs.getTitle() %></a>
								         <input type='hidden' value='<%=bbs.getSeq() %>'>
								       </td> 
								      <td align="center"><%=bbs.getPcategory() %></td>
								      <td align="center"><%=bbs.getNickname() %></td>
								      <td align="center"><%=utilMethod.dateEdit(bbs.getWdate()) %></td>
								      <td align="center"><%=bbs.getRcount() %></td>
								      <td align="center"><%=bbs.getAcount() %></td>
								   <%   
								   }
								}
								%>
								...
								<tr>
									...
									for(int i = 1; i <= totalPages; i++) {
										if(str != null && !str.equals("")) {
											%>
											<a class="paging" href="mypagecontrol?command=mysellbbs&page=<%=i %>&search-select=<%=select %>&search-text=<%=str %>"><%=i %></a>
											<%
										}else {
											%>
											<a class="paging" href="mypagecontrol?command=mysellbbs&page=<%=i %>"><%=i %></a>
											<%
										}
									}
									%>
								...
								<div class="search-div">
									<form id="search-form" action="mypagecontrol">
										<input type="hidden" name="command" value="mysellbbs">
