...
<body class="is-preload">
			...
			<div class="row gtr-200">
				...
				<div class="col-8  col-12-narrower imp-narrower">
					<div id="content">
						<div id = "tab-bar">
							<ul class="tabs">
								<li id="receive">받은쪽지함</li>
								<li class="current" id="send">보낸쪽지함</li>
							</ul>
							<div id="category">
						...
						<div class="msgbox">
							...
							<tr class="tb_head">
								...
								<th>발신일</th>
								<th>수신확인</th>
								<th align="center"><input type="checkbox" id="send-multidel"></th>
							</tr>
							<%
							...
							if(list == null || list.size() == 0) {
								%>
								<tr>
								<td colspan="5" align="center">보낸 메세지가 없습니다</td>
								</tr>
								<%
							}else {
								...
								<tr>
									...
									<td align="center"><%=msg.getTo_nick() %></td>
									...
									<td align="center"><input type="checkbox" class="send-delcheck" name="delcheck" value=<%=msg.getSeq() %>></td>
								</tr>
								<%
								}
							}
							...
							<tr>
								<td colspan="5" align="right">
									...
									<button type="button" id="send-delbtn">쪽지삭제</button>
								</td>
							</tr>
							<tr>
								...
								for(int i = 1; i <= totalPages; i++){
									...
									%>
									<a class="paging" href="mypagecontrol?command=mymsgbox&type=send&page=<%=i %>&search-select=<%=select %>&search-text=<%=str %>"><%=i %></a>
									<%
									...
									%>
									<a class="paging" href="mypagecontrol?command=mymsgbox&type=send&page=<%=i %>"><%=i %></a>
									<%
									}
								}
								%>
	...
	<!-- Scripts -->
		...
		$(function () {
			...
			//	alert($(this).val());
				category = $(this).val();
				location.href="mypagecontrol?command=mymsgbox&type=send&category=" + category;
			});
			...
			$("#send-multidel").click(function () {
				if($("#send-multidel").prop("checked")) {
					$("input:checkbox[class='send-delcheck']").prop("checked", true);
				}else if(!$("#send-multidel").prop("checked")) {
					$("input:checkbox[class='send-delcheck']").prop("checked", false);
				}
			});
			
			// Multi-delete
			$("#send-delbtn").click(function () {
				if(!checkFunc("send-delcheck")) {
					alert("선택한 항목이 없습니다");
					return;
				}
				
				$("#send-msg-form").attr("action", "mypagecontrol").submit();
			});
